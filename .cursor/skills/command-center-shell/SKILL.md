...
2. **Accounting item:** **toggles the Accounting flyout** (`navPanelId === 'accounting'`) and opens the page of whichever settings item the flyout has selected, since the flyout never opens without a selection: `blankCommandCenter` and `settingsShellOpen` go `true` and `settingsPage` becomes `'general-ledger'` unless it is already `'close-agent'`. The dashboard is never what shows behind this flyout. The panel is titled **Accounting** and uses the nested Costpoint tree (General Ledger expanded through General Ledger Controls, then **Configure General Ledger Settings** / **Configure Close Agent Settings**). Selecting either settings item closes the flyout and opens its dedicated page (`settingsPage = 'general-ledger'` or `'close-agent'`), including when it is already the selected item. Plus/minus expanders, pale-blue expanded rows, tree guides, and `#A6C9EC` hover stay on `LeftNavPanel`.
